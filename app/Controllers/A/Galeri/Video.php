...
class Video extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new VideoModel($request);
        ...
        foreach ($lists as $list) {
            ...
            if ($list->image !== null) {
                $image = '<img alt="Image placeholder" src="' . base_url() . '/uploads/video/' . $list->image . '" width="80px" height="50px">';
            } else {
                $image = "-";
            }
            if ($list->url_video !== null) {
                $lampiran = '<a target="_blank" href="' . $list->url_video . '" class="badge badge-pill badge-soft-success">Link Video</a>';
            } else {
                $lampiran = "-";
            }
            $row[] = $action;
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_tb_video')
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                $this->_db->transBegin();
                try {
                    $this->_db->table('_tb_video')->where('id', $id)->delete();

                    if ($this->_db->affectedRows() > 0) {
                        if ($current->image !== null) {
                            try {
                                $dir = FCPATH . "uploads/video";
    ...
    public function addSave()
    {
        ...
        $rules = [
            ...
            '_file' => [
                'rules' => 'uploaded[_file]|max_size[_file,1048]|mime_in[_file,image/jpeg,image/jpg,image/png]',
                'errors' => [
        ...
        if (!$this->validate($rules)) {
            ...
            $response->message = $this->validator->getError('judul')
                . $this->validator->getError('url_video')
            ...
            $url_video = htmlspecialchars($this->request->getVar('url_video'), true);
            $status = htmlspecialchars($this->request->getVar('status'), true);

            $slug = generateSlug($judul);

            $cekData = $this->_db->table('_tb_video')->where(['url' => $slug . '.html'])->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_video')->insert($data);
            } catch (\Exception $e) {
    ...
    public function editSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $oldData =  $this->_db->table('_tb_video')->where('id', $id)->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_video')->where('id', $oldData->id)->update($data);
            } catch (\Exception $e) {
                if ($filenamelampiranFile != '') {
                    unlink($dir . '/' . $newNamelampiranFile);
                }
