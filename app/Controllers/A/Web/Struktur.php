...
class Struktur extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['user'] = $user->data;
        $oldData = $this->_db->table('_web_profil')->where('menu', 'struktur')->get()->getRowObject();
        if ($oldData) {
            $data['data'] = $oldData;
        }
    ...
    public function save()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_web_profil')
                ->where('menu', 'struktur')->get()->getRowObject();

            if ($current) {
                if ($current->isi === $isi) {
                    $response = new \stdClass;
                    $response->status = 201;
                    $response->redirect = base_url('a/web/struktur');
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    $response->status = 200;
                    $response->redirect = base_url('a/web/struktur');
                    $response->message = "Struktur Organisasi Berhasil Disimpan.";
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 400;
