...
class Tugasfungsi extends BaseController
{
    ...
    {
        $data['title'] = 'Tugas & Fungsi Instansi';
        ...
        $data['user'] = $user->data;
        $oldData = $this->_db->table('_web_profil')->where('menu', 'tugasfungsi')->get()->getRowObject();
        if ($oldData) {
            $data['data'] = $oldData;
        }
    ...
    {
        $data['title'] = 'Ubah Tugas & Fungsi Instansi';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            delete_cookie('jwt');
    ...
    public function save()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $data = [
                'isi' => $isi,
                'menu' => 'tugasfungsi',
                'url' => 'tugasfungsi.html',
            ];

            $current = $this->_db->table('_web_profil')
                ->where('menu', 'tugasfungsi')->get()->getRowObject();
