...
class Skm extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['user'] = $user->data;
        $oldData = $this->_db->table('_web_profil')->where('menu', 'skm')->get()->getRowObject();
        if ($oldData) {
            $data['data'] = $oldData;
        }

        return view('a/web/skm/index', $data);
    }

    public function edit()
    {
        $data['title'] = 'Ubah Survei Kepuasan Masyarakat';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            delete_cookie('jwt');
        ...
        return view('a/web/skm/edit', $data);
    }
    ...
    public function save()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_web_profil')
                ->where('menu', 'skm')->get()->getRowObject();

            if ($current) {
                if ($current->isi === $isi) {
                    $response = new \stdClass;
                    $response->status = 201;
                    $response->redirect = base_url('a/web/skm');
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    $response->status = 200;
                    $response->redirect = base_url('a/web/skm');
                    $response->message = "Survei Kepuasan Masyarakat Berhasil Disimpan.";
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 400;
