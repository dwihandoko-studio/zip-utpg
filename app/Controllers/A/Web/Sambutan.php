...
class Sambutan extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['user'] = $user->data;
        $oldData = $this->_db->table('_web_profil')->where('menu', 'sambutan')->get()->getRowObject();
        if ($oldData) {
            $data['data'] = $oldData;
        }
    ...
    {
        $data['title'] = 'Ubah Sambutan Kepala Instansi';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            delete_cookie('jwt');
    ...
    public function save()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_web_profil')
                ->where('menu', 'sambutan')->get()->getRowObject();

            if ($current) {
                if ($current->isi === $isi) {
                    $response = new \stdClass;
                    $response->status = 201;
                    $response->redirect = base_url('a/web/sambutan');
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    $response->status = 200;
                    $response->redirect = base_url('a/web/sambutan');
                    $response->message = "Sambutan Kepala Instansi Berhasil Disimpan.";
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 400;
