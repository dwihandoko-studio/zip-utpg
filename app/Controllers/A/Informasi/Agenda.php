...
use App\Models\A\Informasi\AgendaModel;
use Config\Services;
...
class Agenda extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new AgendaModel($request);
        ...
        foreach ($lists as $list) {
            ...
            if ($list->image !== null) {
                $image = '<img alt="Image placeholder" src="' . base_url() . '/uploads/agenda/' . $list->image . '" width="80px" height="50px">';
            } else {
                $image = "-";
            }
            $row[] = $action;
    ...
    {
        return redirect()->to(base_url('a/informasi/agenda/data'));
    }
    ...
    public function data()
    {
        ...
        return view('a/informasi/agenda/index', $data);
    }
    ...
    public function add()
    {
        ...
        $response->status = 200;
        $response->message = "Permintaan diizinkan";
        $response->data = view('a/informasi/agenda/add');
        return json_encode($response);
    }
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_tb_agenda')
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('a/informasi/agenda/edit', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_tb_agenda a')
                ->where('id', $id)->get()->getRowObject();
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('a/informasi/agenda/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                $this->_db->transBegin();
                try {
                    $this->_db->table('_tb_agenda')->where('id', $id)->delete();

                    if ($this->_db->affectedRows() > 0) {
                        if ($current->image !== null) {
                            try {
                                $dir = FCPATH . "uploads/agenda";
    ...
    public function addSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $cekData = $this->_db->table('_tb_agenda')->where(['url' => $slug . '.html'])->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_agenda')->insert($data);
            } catch (\Exception $e) {
                if ($filenamelampiran != '') {
    ...
    public function editSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_agenda')->where('id', $oldData->id)->update($data);
            } catch (\Exception $e) {
                if ($filenamelampiran != '') {
                    unlink($dir . '/' . $newNamelampiran);
                }
    ...
    public function uploadImage()
    {
        ...
        if ($validated) {
            ...
            $filesNamelampiran = $lampiran->getName();
            $newNamelampiran = _create_name_foto($filesNamelampiran);
            $writePath = FCPATH . "uploads/agenda/widget";

            if ($lampiran->isValid() && !$lampiran->hasMoved()) {
                $lampiran->move($writePath, $newNamelampiran);
                $data = [
                    "uploaded" => true,
                    "url" => base_url('uploads/agenda/widget/' . $newNamelampiran),
                ];
            } else {
                $data = [
                    "uploaded" => false,
