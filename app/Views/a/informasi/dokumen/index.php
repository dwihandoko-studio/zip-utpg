...
<script>
    function actionHapus(event, title = "") {
        <?php if (listHakAksesCustomAllowNew('informasi', 'dokumen', 'delete')) { ?>
            Swal.fire({
                title: 'Apakah anda yakin ingin menghapus data ini?',
                text: "Hapus Dokumen : " + title,
                ...
                if (result.value) {
                    $.ajax({
                        url: "<?= base_url('a/informasi/dokumen/delete') ?>",
                        type: 'POST',
                        data: {
                            id: event,
                        },
    ...
    function actionEdit(event, title) {
        <?php if (listHakAksesCustomAllowNew('informasi', 'dokumen', 'edit')) { ?>
            $.ajax({
                url: "<?= base_url('a/informasi/dokumen/edit') ?>",
                type: 'POST',
                data: {
                    id: event,
                },
                ...
                success: function(resul) {
                    ...
                    if (resul.status !== 200) {
                        ...
                        $('#content-detailModalLabel').html('EDIT DOKUMEN ' + title);
    ...
    function actionAdd(event) {
        $.ajax({
            url: "<?= base_url('a/informasi/dokumen/add') ?>",
            type: 'GET',
            dataType: 'JSON',
            beforeSend: function() {
                $('div.main-content').block({
            ...
            success: function(resul) {
                ...
                if (resul.status !== 200) {
                    ...
                    $('#content-detailModalLabel').html('TAMBAH DOKUMEN');
                    $('.contentBodyModal').html(resul.data);
                    $('.content-detailModal').modal({
                        backdrop: 'static',
                        keyboard: false,
    ...
    $(document).ready(function() {
        ...
        let tableDatatables = $('#data-datatables').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('a/informasi/dokumen/getAll') ?>",
                "type": "POST",
