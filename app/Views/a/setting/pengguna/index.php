...
<script>
    function actionResetAkun(event, title = "") {
        <?php if (listHakAksesCustomAllowNew('setting', 'pengguna', 'reset')) { ?>
            Swal.fire({
                title: 'Apakah anda yakin ingin mereset akun pengguna ini?',
                text: "Reset Akun Pengguna : " + title,
    ...
    function actionHapus(event, title = "") {
        <?php if (listHakAksesCustomAllowNew('setting', 'pengguna', 'delete')) { ?>
            Swal.fire({
            ...
            $.ajax({
                ...
                success: function(resul) {
                    ...
                    if (resul.status !== 200) {
                        ...
                        $('#content-detailModalLabel').html('EDIT PENGGUNA ' + title);
