        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">FOTO</h4>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <?php if (listHakAksesCustomAllowNew('galeri', 'foto', 'add')) { ?>
...
<script>
    function actionHapus(event, title = "") {
        <?php if (listHakAksesCustomAllowNew('galeri', 'foto', 'delete')) { ?>
            Swal.fire({
                title: 'Apakah anda yakin ingin menghapus data ini?',
                text: "Hapus Foto : " + title,
                ...
                if (result.value) {
                    $.ajax({
                        url: "<?= base_url('a/galeri/foto/delete') ?>",
                        type: 'POST',
                        data: {
                            id: event,
                        },
    ...
    function actionEdit(event, title) {
        <?php if (listHakAksesCustomAllowNew('galeri', 'foto', 'edit')) { ?>
            $.ajax({
                url: "<?= base_url('a/galeri/foto/edit') ?>",
                type: 'POST',
                data: {
                    id: event,
                },
                ...
                success: function(resul) {
                    ...
                    if (resul.status !== 200) {
                        ...
                        $('#content-detailModalLabel').html('EDIT FOTO ' + title);
    ...
    function actionAdd(event) {
        $.ajax({
            url: "<?= base_url('a/galeri/foto/add') ?>",
            type: 'GET',
            dataType: 'JSON',
            beforeSend: function() {
                $('div.main-content').block({
            ...
            success: function(resul) {
                ...
                if (resul.status !== 200) {
                    ...
                    $('#content-detailModalLabel').html('TAMBAH FOTO');
                    $('.contentBodyModal').html(resul.data);
                    $('.content-detailModal').modal({
                        backdrop: 'static',
                        keyboard: false,
    ...
    $(document).ready(function() {
        ...
        let tableDatatables = $('#data-datatables').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('a/galeri/foto/getAll') ?>",
                "type": "POST",
