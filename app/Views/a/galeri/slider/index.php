                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <?php if (listHakAksesCustomAllowNew('galeri', 'slider', 'add')) { ?>
...
<script>
    function actionHapus(event, title = "") {
        <?php if (listHakAksesCustomAllowNew('galeri', 'slider', 'delete')) { ?>
            Swal.fire({
                title: 'Apakah anda yakin ingin menghapus data ini?',
                text: "Hapus Slider : " + title,
                ...
                if (result.value) {
                    $.ajax({
                        url: "<?= base_url('a/galeri/slider/delete') ?>",
                        type: 'POST',
                        data: {
                            id: event,
                        },
    ...
    function actionEdit(event, title) {
        <?php if (listHakAksesCustomAllowNew('galeri', 'slider', 'edit')) { ?>
            $.ajax({
                url: "<?= base_url('a/galeri/slider/edit') ?>",
                type: 'POST',
                data: {
                    id: event,
                },
                ...
                success: function(resul) {
                    ...
                    if (resul.status !== 200) {
                        ...
                        $('#content-detailModalLabel').html('EDIT SLIDER ' + title);
    ...
    function actionAdd(event) {
        $.ajax({
            url: "<?= base_url('a/galeri/slider/add') ?>",
            type: 'GET',
            dataType: 'JSON',
            beforeSend: function() {
                $('div.main-content').block({
            ...
            success: function(resul) {
                ...
                if (resul.status !== 200) {
                    ...
                    $('#content-detailModalLabel').html('TAMBAH SLIDER');
                    $('.contentBodyModal').html(resul.data);
                    $('.content-detailModal').modal({
                        backdrop: 'static',
                        keyboard: false,
    ...
    $(document).ready(function() {
        ...
        let tableDatatables = $('#data-datatables').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('a/galeri/slider/getAll') ?>",
                "type": "POST",
