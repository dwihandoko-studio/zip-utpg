<?php if (isset($data)) { ?>
    <form id="formEditModalData" action="./editSave" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="col-lg-12">
                    <label for="_url_video" class="col-form-label">URL Video:</label>
                    <input type="text" class="form-control url_video" value="<?= $data->url_video ?>" id="_url_video" name="_url_video" placeholder="Url video..." onfocusin="inputFocus(this);">
                    <div class="help-block _url_video"></div>
                </div>
                <div class="col-lg-12 mt-4">
                    <div class="row mt-4">
                        <div class="col-lg-6">
                            <div class="mt-3">
                                <label for="_file" class="form-label">Gambar Thumb Video: </label>
                            ...
                            <div class="form-group">
                                <div class="preview-image-upload">
                                    <?php if ($data->image !== null) { ?>
                                        <img class="imagePreviewUpload" src="<?= base_url('uploads/video') . '/' . $data->image ?>" id="imagePreviewUpload" />
                                    <?php } else { ?>
    ...
    <script>
        ...
        $("#formEditModalData").on("submit", function(e) {
            ...
            const url_video = document.getElementsByName('_url_video')[0].value;
            ...
            if (judul.length > 200) {
                $("input#_judul").css("color", "#dc3545");
                $("input#_judul").css("border-color", "#dc3545");
                $('._judul').html('<ul role="alert" style="color: #dc3545; list-style-type:none; padding-inline-start: 10px;"><li style="color: #dc3545;">Judul maksimal 200 karakter.</li></ul>');
                return false;
            }

            if (url_video === "") {
                $("input#_url_video").css("color", "#dc3545");
                $("input#_url_video").css("border-color", "#dc3545");
                $('._url_video').html('<ul role="alert" style="color: #dc3545; list-style-type:none; padding-inline-start: 10px;"><li style="color: #dc3545;">Sub judul tidak boleh kosong.</li></ul>');
                return false;
            }

            if (url_video.length < 5) {
                $("input#_url_video").css("color", "#dc3545");
                $("input#_url_video").css("border-color", "#dc3545");
                $('._url_video').html('<ul role="alert" style="color: #dc3545; list-style-type:none; padding-inline-start: 10px;"><li style="color: #dc3545;">Sub judul minimal 5 karakter.</li></ul>');
                return false;
            }

            if (url_video.length > 250) {
                $("input#_url_video").css("color", "#dc3545");
                $("input#_url_video").css("border-color", "#dc3545");
                $('._url_video').html('<ul role="alert" style="color: #dc3545; list-style-type:none; padding-inline-start: 10px;"><li style="color: #dc3545;">Sub judul maksimal 250 karakter.</li></ul>');
                return false;
            }
            ...
            formUpload.append('url_video', url_video);
